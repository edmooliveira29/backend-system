...
import { CompanyRepositoryInfra } from './company-repository'
...
describe('Mongodb User repository', () => {
  ...
  test('Should created a new user', async () => {
    const sut = new CompanyRepositoryInfra()
    const userAdded = await sut.createCompany({
      _id: 'anyId',
      name: 'anyName',
  ...
  test('Should return error if exist user with same email', async (): Promise<void> => {
    ...
    await sut.createCompany(userMock)
    expect(async () => await sut.createCompany(userMock)).rejects.toStrictEqual(new Error('Já existe um usuário com este e-mail'))
  })
  ...
  test('Return true with authentication with sucessfuly', async () => {
    ...
    jest.spyOn(sut, 'findCompanyByEmailOrId').mockResolvedValue(userMock)
